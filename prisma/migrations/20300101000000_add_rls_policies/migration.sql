...
CREATE POLICY "company_select_own" ON "Company"
  FOR SELECT USING (
    id IN (SELECT "companyId" FROM "User" WHERE "id"::uuid = auth.uid())
  );
...
CREATE POLICY "user_select_same_company" ON "User"
  FOR SELECT USING (
    "companyId" = (SELECT "companyId" FROM "User" WHERE "id"::uuid = auth.uid())
  );
  ...
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE c.id = "ShareLink"."candidateId"
        AND c."companyId" = (SELECT "companyId" FROM "User" WHERE "id"::uuid = auth.uid())
    )
  );
