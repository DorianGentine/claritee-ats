...
describe.runIf(!!connectionString)("search", () => {
  ...
  beforeAll(async () => {
    ...
    const offer = await db.jobOffer.create({
      data: {
        ...
        description: "Nous recherchons un expert React pour notre équipe produit.",
        companyId: companyAId,
  ...
  it("search: finds candidate by title", async () => {
    ...
    expect(result.candidates.length).toBeGreaterThanOrEqual(1)
    expect(result.candidates.some((c) => c.title?.includes("React"))).toBe(true)
  })
  ...
  it("search: finds offer by title", async () => {
    ...
    expect(result.offers.find((o) => o.id === offerAId)?.clientCompany?.name).toBe(
      "Acme Corp"
    )
  })
