...
- [ ] Task 1 (AC: 9): API – Router client
  - [ ] Créer `src/server/trpc/routers/client.ts` et l'enregistrer dans `_app.ts`. [Source: docs/architecture/coding-standards.md §3.1, docs/architecture.md §9]
  - [ ] `list` : protectedProcedure, where companyId = ctx.companyId, orderBy createdAt desc. Retourner id, name, siren, _count: { contacts: true, jobOffers: true }. [Source: prisma/schema.prisma ClientCompany]
  - [ ] `getById` : protectedProcedure, input { id }, vérifier companyId. Include contacts count, jobOffers count. [Source: docs/architecture.md §4]
  - [ ] `create` : protectedProcedure, input validé par createClientCompanySchema. [Source: Task 2]
- [ ] Task 2 (AC: 5, 6): Validations
  - [ ] Créer `src/lib/validations/client.ts` : createClientCompanySchema avec name (string min 1), siren (optional, 9 chiffres). Réutiliser sirenSchema ou pattern depuis auth. [Source: docs/prd.md Standard Error Messages, src/lib/validations/auth.ts]
- [ ] Task 3 (AC: 1, 2, 3, 7, 8, 10): Page liste clients
  - [ ] Créer `src/app/(dashboard)/clients/page.tsx`. [Source: docs/architecture/source-tree.md]
  - [ ] Afficher la liste via client.list ; cartes ou table : nom, SIREN, "X contacts · Y offres", clic → `/clients/[id]`. [Source: wireframes §6]
  - [ ] Empty state "Aucun client ajouté". [Source: AC 8]
  - [ ] Loading skeleton pendant le fetch. [Source: AC 10]
- [ ] Task 4 (AC: 4, 5, 6): Création client
  - [ ] Bouton "Nouveau client" (terracotta). [Source: AC 4]
  - [ ] Modal ou page dédiée avec formulaire : companyName* (raison sociale), siren (optionnel). [Source: AC 5]
  - [ ] Validation SIREN si renseigné (9 chiffres). [Source: AC 6]
  - [ ] Mutation client.create ; invalider client.list ; rediriger vers fiche client ou fermer modal et rafraîchir la liste. [Source: docs/frontend-architecture.md §5]
- [ ] Task 5 (AC: 7): Page détail client (minimale)
  - [ ] Créer `src/app/(dashboard)/clients/[id]/page.tsx`. Afficher nom, SIREN ; sections Contacts et Offres en placeholder ou vides (Story 3.2 pour les contacts). [Source: wireframes §7]
  - [ ] Vérifier que le client appartient au cabinet (getById 404 sinon). [Source: docs/architecture.md §4]
- [ ] Task 6: Navigation
  - [ ] Activer le lien "Clients" dans la navbar : href="/clients", retirer "Clients (Bientôt)" et disabled. [Source: src/components/layout/SiteNavbar.tsx]
- [ ] Task 7 – Testing (AC: 1–10)
  - [ ] Tests d'intégration : client.list (scopé companyId), client.create (validation SIREN), client.getById (NOT_FOUND autre cabinet). [Source: docs/architecture.md §14]
  - [ ] Test manuel : liste, création, empty state, navigation vers détail.
- [ ] Task 8 (QA): Rédiger les cas de test dans un fichier dédié (ex. docs/stories/cases/3.1-test-cases.md) pour faciliter la review de la story.
- [ ] Task 9 (review): Lancer une review de code en fin de développement avant clôture de la story.
