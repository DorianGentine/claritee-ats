...
- [x] Task 3 (AC: 5, 6, 7): Implement registration flow (Supabase Auth + Company + User)
  ...
  - [x] Error messages: « Cette combinaison n'est pas disponible. Utilisez un autre email ou un autre numéro SIREN. » (SIREN ou email déjà utilisé). [Source: docs/prd.md Standard Error Messages]
...
- **Errors:** Use TRPCError with code and message; client displays message. Inscription (SIREN/email indisponible), Format SIREN invalide, Format email invalide per PRD. [Source: docs/prd.md Standard Error Messages]
...
### Troubleshooting — DATABASE_URL et caractères spéciaux (`$`)

- **Problème** : Si le mot de passe de `DATABASE_URL` contient un `$`, Next.js (via `dotenv-expand`) interprète `$xxx` comme une variable d'environnement. La valeur est tronquée ou modifiée → erreur « Authentication failed against the database server ».
- **Prisma migrate/seed** : utilise dotenv simple, donc le `$` n'est pas expandé et la connexion fonctionne.
- **Next.js** : charge `.env.local` avec expansion de variables → le mot de passe est corrompu au chargement.
- **Solutions** :
  1. Échapper le `$` avec un backslash : `DATABASE_URL="postgresql://user:pass\$word@host..."`
  2. Ou encoder en URL : remplacer `$` par `%24` dans la partie mot de passe : `pass%24word` (Postgres décode correctement).
...
| 2026-02-14 | 0.3     | Refactor: single auth.register (Supabase Admin); doc troubleshooting DATABASE_URL + `$` dans mdp | James (Dev) |
...
- Registration flow: single tRPC `auth.register` (validate SIREN + email, create auth via Supabase Admin, create Company + User in one transaction); client then calls `signInWithPassword`.
- **DATABASE_URL** : si le mot de passe contient un `$`, échapper avec `\$` ou remplacer par `%24` dans `.env.local` (Next.js utilise dotenv-expand qui interprète `$` comme variable).
- Context extended with headers for rate limiting.
