...
- Arbo des tests sous `__tests__/` (racine), calquée sur `/src`, splittée par dépendance à la DB :
  - `__tests__/db/**` — tests avec Prisma réel (`pnpm test:db`, base `claritee_test`, en série)
  - `__tests__/unit/**` — tests sans DB (`pnpm test:unit`), dont les composants sous `unit/components/<domaine>/`
