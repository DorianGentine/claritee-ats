...
export const getNoteExcerpt = (content: string): string => {
  try {
    ...
    if (!plain) return "";
    return plain.length > EXCERPT_MAX_LEN
      ? plain.slice(0, EXCERPT_MAX_LEN) + "…"
      : plain;
  } catch {
    return "";
  }
...
export const getNoteDisplayTitle = (
  ...
  const t = title?.trim();
  if (t)
    return t.length > DISPLAY_TITLE_MAX_LEN
      ? t.slice(0, DISPLAY_TITLE_MAX_LEN) + "…"
      : t;
  try {
    const parsed = JSON.parse(content) as unknown;
    const text = extractTextFromBlocks(
    ...
    return plain.length > DISPLAY_TITLE_MAX_LEN
      ? plain.slice(0, DISPLAY_TITLE_MAX_LEN) + "…"
      : plain;
  } catch {
    return "Sans titre";
  }
