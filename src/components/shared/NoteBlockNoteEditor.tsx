...
export const isBlockNoteContentEmpty = (content: string): boolean => {
  ...
  try {
    ...
    const hasText = (block: {
      ...
      if (
        block.content?.some((c) => c?.text && String(c.text).trim().length > 0)
      )
        return true;
...
export const NoteBlockNoteEditor = forwardRef<NoteBlockNoteEditorRef, Props>(
  (
    { initialContent, editable = true, onContentChange, onRawContentChange },
    ref
  ) => {
    const initialBlocks = (() => {
    ...
    return (
      <div
        className={`p-2 bg-white rounded-md ${editable ? "min-h-[8em]" : ""}`}
      >
        <BlockNoteView
          editor={editor}
          editable={editable}
